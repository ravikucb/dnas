...
# This nas_searchmanager.py is for the sample
# extension and is customized for the CNN search
# space in that extension.
...
class SearchManager(object):
    ...
    def move_data_to_gpu(self, images, labels, device):
        return images.to(device), labels.to(device)

    def run_one_dnas_step(self, current_epoch, batch_idx, curr_temp, images, labels,
            weights_optimizer, mask_optimizer):
        # Move the data and target
        # tensors to self.host_device.
        images, labels = self.move_data_to_gpu(images, labels, self.host_device)
        ...
        super_net_outputs = self.super_net(images,
                                sampling="soft",
                                temperature=curr_temp)
        ...
        if self.use_hw_cost is True:
            ...
            if self.exponential_cost:
                ...
                loss = loss + (self.cost_coef * mean_log_cost)

        # Backpropagation.
        loss.backward()
        clip_grad(self.super_net.parameters(), self.clip_grad_norm_value)
        #print("DNAS TRAINING:", batch_idx, loss.item())
    ...
    def train_dnas(self):
        ...
        for current_epoch in all_epochs:
            ...
            # Training loop.
            for batch_idx, (images, labels) in enumerate(current_dataloader):
                # Check if we need to skip this
                # training step; if so, skip it.
                if batch_idx < steps_to_skip:
                    continue

                # If the batch size is not the same, skip (e.g. last batch).
                if list(images.size())[0] != current_dataloader.batch_size:
                    continue

                # Run one training step.
                curr_loss_value = self.run_one_dnas_step(current_epoch, batch_idx,
                    curr_temp, images, labels, weights_optimizer, mask_optimizer)
