...
class DLRM_Net(nn.Module):
    ...
    def create_emb(self, m, ln):
        ...
        for i in range(0, ln.size):
            ...
            if self.qr_flag and n > self.qr_threshold:
                ...
                # Current m to support multiple dimensions depending on self.max_emb_dim.
                m_curr = (m[i] if self.max_emb_dim > 0 else m)

                # Support multiple dimensions depending on self.max_emb_dim.
    ...
    def apply_emb(self, lS_o, lS_i, emb_l):
        ...
        for k in range(len(lS_i)):
            # We hash the indices to the correct number. This is not necessary unless we are using the emb_card search space in which we pass the unhashed dataset in,
            # but we may have an embedding matrix that is smaller than that. Hashing will have no effect if this is not the case (i.e. if embedding num vectors = max index).
            sparse_index_group_batch = lS_i[k] % emb_l[k].num_embeddings
            ...
            # To support multiple embedding dimensions.
            if self.max_emb_dim > 0:
                # Zero-pad to maximum dimension.
                V = F.pad(V, (0, (self.max_emb_dim - E.embedding_dim)))
